...
@router.patch("/users/me/co-organizers/{co_organizer_id}", status_code=status.HTTP_200_OK)
async def update_create_co_organizer_status(co_organizer_id: int, create_co_organizer: bool, user=Depends(require_user)):
...
@router.get("/users/me/events/co-organizing", response_model=list[EventOut], status_code=status.HTTP_200_OK)
async def get_user_co_organizing_events(user=Depends(require_user)):
    """List all events that a user is co-organizing."""
    co_organizing = await co_services.get_user_co_organizing_events_service(user.id)
