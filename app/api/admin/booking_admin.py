...
from app.schemas.booking import BookingOut, BookingUpdate, BookingEnrichedOut
...
@router.get("/admin/bookings", response_model=list[BookingEnrichedOut])
async def list_bookings(user=Depends(require_admin)):
    """List all bookings with customer, event, and ticket type details."""
    return await booking_services.list_bookings_enriched_service()
...
async def list_recent_bookings(limit: int = 10, user=Depends(require_admin)):
    ...
    return await booking_services.list_recent_bookings_service(limit)
...
async def list_bookings_by_status(status: str, user=Depends(require_admin)):
    ...
    return await booking_services.list_bookings_by_status_service(status)
...
async def list_bookings_by_user(user_id: int, user=Depends(require_admin)):
    ...
    return await booking_services.list_bookings_by_user_service(user_id)
...
async def list_bookings_by_user_and_status(user_id: int, status: str, user=Depends(require_admin)):
    ...
    return await booking_services.list_bookings_status_by_user_service(user_id, status)

@router.get("/admin/bookings/ticket_type/{ticket_type_id}/status/{status}", response_model=list[BookingOut])
async def list_bookings_by_ticket_type_and_status(
    ticket_type_id: int, status: str, user=Depends(require_admin)
):
    """List all bookings for a specific ticket type with a specific status."""
    return await booking_services.list_bookings_by_ticket_type_and_status_service(ticket_type_id, status)

@router.get("/admin/bookings/date-range/{start_date}/{end_date}", response_model=list[BookingOut])
async def list_bookings_in_date_range(
...
async def update_booking(
    booking_id: int,
    booking: BookingUpdate,          # was BookingOut — wrong input schema
    ...
    if res is not None:
        background_tasks.add_task(
            ...
            details={"quantity": booking.quantity, "status": booking.status},
        )

    return res

@router.patch("/admin/bookings/{booking_id}/status", status_code=status.HTTP_204_NO_CONTENT)
async def update_booking_status(
    booking_id: int,
    booking_status: str,             # renamed from `status` to avoid shadowing fastapi.status
    background_tasks: BackgroundTasks,
    user=Depends(require_admin),
):
    """Update the status of an existing booking."""
    await booking_services.update_booking_status_service(booking_id, booking_status)
    ...
    background_tasks.add_task(
        ...
        target_type="booking",
        target_id=booking_id,
        details={"updated_status": booking_status},
    )

@router.delete("/admin/bookings/{booking_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_booking(
    booking_id: int, background_tasks: BackgroundTasks, user=Depends(require_admin)
):
    """Delete a specific booking by its ID."""
    await booking_services.delete_booking_service(booking_id)
