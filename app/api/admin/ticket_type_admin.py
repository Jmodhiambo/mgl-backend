...
async def tt_admin_create_ticket_type(ticket_type_in: TicketTypeCreate, background_tasks: BackgroundTasks, admin=Depends(require_admin)):
    """Create a new TicketType."""
    ticket_type = await tt_services.create_ticket_type_service(ticket_type_in)

    # Create admin log entry for ticket type creation
...
async def tt_admin_update_ticket_type(ticket_type_id: int, ticket_type_in: TicketTypeUpdate, background_tasks: BackgroundTasks, admin=Depends(require_admin)):
    ...
    # Create admin log entry for ticket type update
    background_tasks.add_task(
        log_admin_action_service,
        admin_id=admin.id,
        admin_name=admin.name,
        ...
        target_type="ticket_type",
        target_id=ticket_type.id,
        details={"admin": admin.name, "status": ticket_type_in.status},
    )
    return ticket_type

@router.delete("/admin/ticket-types/{ticket_type_id}", response_model=dict)
async def tt_admin_delete_ticket_type(ticket_type_id: int, background_tasks: BackgroundTasks, admin=Depends(require_admin)):
    """Delete a TicketType by ID."""
    success = tt_services.delete_ticket_type_service(ticket_type_id)
    if not success:
        raise HTTPException(status_code=404, detail="TicketType not found")
    
    # Create admin log entry for ticket type deletion
    background_tasks.add_task(
        log_admin_action_service,
        admin_id=admin.id,
        admin_name=admin.name,
