...
def create_payment(user_id: int, payment: PaymentCreate, user=Depends(get_current_user)):
...
def get_payment_by_id(user_id: int, payment_id: int, user=Depends(get_current_user)):
    """Get a payment by ID."""
    if user.id != user_id:
...
def get_payments_by_booking(user_id: int, booking_id: int, user=Depends(get_current_user)):
...
def update_payment_by_id(user_id: int, payment_id: int, payment_update: PaymentUpdate, user=Depends(get_current_user)):
