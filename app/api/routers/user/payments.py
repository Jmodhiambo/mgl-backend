...
async def create_payment(user_id: int, payment: PaymentCreate, user=Depends(require_user)):
...
async def get_payment_by_id(user_id: int, payment_id: int, user=Depends(require_user)):
    """Get a payment by ID."""
    if user.id != user_id:
...
async def get_payments_by_booking(user_id: int, booking_id: int, user=Depends(require_user)):
...
async def update_payment_by_id(user_id: int, payment_id: int, payment_update: PaymentUpdate, user=Depends(require_user)):
